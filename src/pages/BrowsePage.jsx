  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    if (filters.content_type?.length > 0) {
      results = results.filter((item) =>
        filters.content_type.includes(item.content_type)
      )
    }
    ...
    if (filters.difficulty_level?.length > 0) {
      results = results.filter((item) =>
        filters.difficulty_level.includes(item.difficulty_level)
      )
    }

    if (filters.source_platform?.length > 0) {
      results = results.filter((item) =>
        filters.source_platform.includes(item.source_platform)
      )
    }
    ...
    if (filters.price_range) {
      results = results.filter((item) => {
        ...
        return (
          item.price >= filters.price_range[0] &&
          item.price <= filters.price_range[1]
        )
      })
    }
    ...
    if (filters.quality_score_min > 0) {
      results = results.filter(
        (item) => item.quality_score >= filters.quality_score_min
      )
    }
  ...
  return (
    <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
      ...
      <div className='flex gap-8'>
        {/* Sidebar */}
        <div
          className={`${isSidebarOpen ? 'w-80' : 'w-0'} transition-all duration-300 overflow-hidden flex-shrink-0`}
        >
          <FilterSidebar
            filters={filters}
