...
const ContentCard = ({ item, onView, className = '' }) => {
  ...
  return (
    <Card className={`h-full flex flex-col ${className}`} hover>
      {/* Header */}
      <div className='flex items-start justify-between mb-3'>
        <div className='flex items-center space-x-2'>
          ...
          <span
            className={`px-2 py-1 text-xs font-medium rounded-full ${getDifficultyColor(difficulty_level)}`}
          >
            {difficulty_level}
          </span>
        </div>
      ...
      <div className='mt-4 pt-3 border-t border-white/10'>
        <Button onClick={() => onView(item)} className='w-full' size='sm'>
          View Details
        </Button>
      </div>
    </Card>
